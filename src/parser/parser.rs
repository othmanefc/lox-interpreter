        ...
        let expr =
            match &token.token_type {
                ...
                TokenType::Minus | TokenType::Bang => {
                    let mut unary_ops = vec![token.clone()];
                    let mut final_expr = None;

                    while let Some(next_token) = tokens_iter.next() {
                        match &next_token.token_type {
                            TokenType::Minus | TokenType::Bang => {
                                unary_ops.push(next_token.clone());
                            }
                            _ => {
                                // This token is not a unary operator, so it's the operand
                                let vec_token = vec![next_token.clone()];
                                if let Some(expr) = parse_tokens(&vec_token).into_iter().next() {
                    ...
                    if let Some(expr) = final_expr {
                        // Build the nested unary expressions from right to left
                        Some(unary_ops.into_iter().rev().fold(expr.unwrap(), |acc, op| {
                            Expr::Unary {
                                operator: op,
                                right: Box::new(acc),
                            }
                        ...
                        None // Or handle this error case as appropriate
                    }
                }
                // TokenType::Minus | TokenType::Bang => {
